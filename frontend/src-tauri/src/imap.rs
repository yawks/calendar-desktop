...
fn parse_recipient(s: &str) -> MailRecipient {
    ...
    MailRecipient { name: None, email: s.to_string() }
}
...
impl MailProvider for ImapProvider {
    ...
    async fn get_inbox_unread(&self) -> Result<u32, String> {
        let folder = self.folder.as_deref().ok_or_else(|| "IMAP folder required".to_string())?;
        let mut session = get_imap_session(&self.config).await?;
    ...
    async fn mark_read(&self, items: &[MailItemRef]) -> Result<(), String> {
        ...
        let query = items.iter().map(|i| i.item_id.as_str()).collect::<Vec<_>>().join(",");
    ...
    async fn move_to_trash(&self, item_id: &str) -> Result<(), String> {
        ...
        session.copy(item_id, &trash).await.map_err(|e| format!("IMAP copy error: {}", e))?;
        session.store(item_id, "+FLAGS (\\Deleted)").await.map_err(|e| format!("IMAP store error: {}", e))?
            .collect::<Vec<_>>().await;
    ...
    async fn send_mail(&self, params: SendMailParams) -> Result<(), String> {
        let mut email_builder = Message::builder()
            .from(self.config.email.parse().map_err(|e| format!("Invalid from address: {}", e))?)
...
pub async fn imap_get_inbox_unread(config: ImapConfig, folder: String) -> Result<u32, String> {
    ImapProvider::with_folder(config, folder).get_inbox_unread().await
}
...
pub async fn imap_mark_read(config: ImapConfig, folder: String, ids: Vec<String>) -> Result<(), String> {
    let items: Vec<MailItemRef> = ids.into_iter()
        .map(|id| MailItemRef { item_id: id, change_key: String::new(), conversation_id: None, folder: Some(folder.clone()) })
        .collect();
...
pub async fn imap_send(
    ...
    ImapProvider::new(config).send_mail(SendMailParams {
        to,
        cc,
        bcc,
        subject,
...
pub async fn imap_get_attachment_data(
    ...
    ImapProvider::new(config).get_attachment_data(&attachment_id, Some(&message_id), Some(&folder)).await
}
