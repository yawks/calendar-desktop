...
interface MailSidebarProps {
  ...
  readonly allAccountFolders: Map<string, MailFolder[]>;
  readonly allMailAccounts: any[];
}
...
function buildFolderTree(folders: DynamicFolderEntry[]): FolderNode[] {
  ...
  for (const [name, node] of Array.from(nodeMap.entries())) {
    const parts = name.split('/');
    if (parts.length === 1) {
      roots.push(node);
    } else {
...
function FolderTreeNode({
  ...
  return (
    <>
      ...
      >
        <button className="mail-folder-main" onClick={handleMainClick} type="button">
...
export function MailSidebar({
  ...
  return (
    <nav className="mail-sidebar">
      <button className="mail-compose-btn" onClick={onCompose} type="button">
        <Pencil size={15} />
