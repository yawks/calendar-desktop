...
export async function patchCachedEventRsvp(
  ...
  try {
    const db = await openDB();
    const entry = await new Promise<CacheEntry<import('../types').CalendarEvent[]> | undefined>((resolve) => {
      const tx = db.transaction(STORE, 'readonly');
      const req = tx.objectStore(STORE).get(key);
      req.onsuccess = () => resolve(req.result as CacheEntry<import('../types').CalendarEvent[]> | undefined);
      req.onerror = () => resolve(undefined);
    });
    if (!entry) return;
    ...
    await new Promise<void>((resolve) => {
      const tx = db.transaction(STORE, 'readwrite');
      tx.objectStore(STORE).put({ value: patched, at: entry.at } satisfies CacheEntry<import('../types').CalendarEvent[]>, key);
      tx.oncomplete = () => resolve();
      tx.onerror = () => resolve();
    });
  } catch {
