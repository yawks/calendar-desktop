...
class SyncBehaviorTest{
 ...
 @Test fun credentialUpdatesAreMergedWithoutDroppingRefreshToken(){val a=SyncAccount("a","gmail","a@b.c",null,"https://example.test",JSONObject().put("refreshToken","r"));val updated=a.withCredentialUpdate(JSONObject().put("accessToken","new"));assertEquals("r",updated.credentials.getString("refreshToken"));assertEquals("new",updated.credentials.getString("accessToken"))}
 @Test fun allProvidersUseTheSameAccountContract(){for(provider in listOf("gmail","exchange","imap","jmap"))assertEquals(provider,SyncAccount("a",provider,"a@b.c",null,"https://example.test",JSONObject()).provider)}
